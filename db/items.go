...
// ListFolderItems lists the folders and files directly inside parentID (or
// the root, if parentID is "" or the root sentinel), owned by ownerID, as a
// single deterministically-ordered, paginated list — combining what were
// previously two separately-paginated queries (ListChildFolder,
...
func ListFolderItems(parentID, ownerID, sortBy string, sortDir sdk.SortDirection, limit, offset int) ([]sdk.FolderItem, error) {
	...
	if parentID == "" || parentID == rootFolderID {
		...
		`+orderBy+` LIMIT $2 OFFSET $3`, ownerID, limit, offset)
	} else {
		rows, err = DB.Query(`
			SELECT * FROM (
				SELECT 'folder' AS item_type, f.id, f.uuid, f.name, '' AS extension, '' AS mime_type,
		...
		`+orderBy+` LIMIT $3 OFFSET $4`, parentID, ownerID, limit, offset)
	}
	if err != nil {
		return nil, err
	}
