...
func CreateShareFolderLink(folderUUID, createdBy string, expiresAt *time.Time, requireLogin bool) (ShareFolderLink, error) {
	// reuse the same random charset as file share tokens
	...
	err = DB.QueryRow(`
		WITH ins AS (
			INSERT INTO share_folder_links (token, folder_id, created_by, expires_at, require_login)
			VALUES ($1, (SELECT id FROM folders WHERE uuid=$2), $3::BIGINT, $4, $5)
			RETURNING id, token, folder_id, created_by, expires_at, created_at, require_login
		)
		SELECT ins.id, ins.token, ins.folder_id, fo.uuid, fo.name,
		       ins.created_by, ins.expires_at, ins.created_at, ins.require_login
		FROM ins
		JOIN folders fo ON fo.id = ins.folder_id
	`, token, folderUUID, createdBy, expiresAt, requireLogin).Scan(
